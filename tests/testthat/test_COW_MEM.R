...
test_that("Column `Beg` has standardised dates", {
  expect_equal(class(memberships[["COW_MEM"]]$Beg), "mdate")
  expect_false(any(grepl("/", memberships[["COW_MEM"]]$Beg)))
  expect_false(any(grepl("^[:alpha:]$",
                         memberships[["COW_MEM"]]$Beg)))
...
test_that("dataset is arranged by the `Beg` variable", {
  expect_true(memberships[["COW_MEM"]]$Beg[1] <
                memberships[["COW_MEM"]]$Beg[100])
  expect_true(memberships[["COW_MEM"]]$Beg[120] <
                memberships[["COW_MEM"]]$Beg[220])
  expect_true(memberships[["COW_MEM"]]$Beg[250] <
                memberships[["COW_MEM"]]$Beg[350])
})
